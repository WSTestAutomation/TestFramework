...
class ReportTestResult(unittest.TestResult):
    ...
    def stopTest(self, test) -> None:
        ...
        self.end_time = '{} s'.format((datetime.datetime.now() - self.start_time).seconds)
    ...
    def stopTestRun(self, title=None) -> dict:
        ...
        FIELDS['testSkip'] = self.skipped_count
        self.FIELDS = FIELDS
        return FIELDS
    ...
    def addError(self, test, err):
        ...
        self.errors.append((test, self.error_or_failure_text(err)))
    ...
    def addFailure(self, test, err):
        ...
        logs.extend(self.error_or_failure_text(err))
        self.failure_count += 1
        self.failures.append((test, self.error_or_failure_text(err)))
        self.add_test_type('失败', logs)
        if self.verbosity > 1:
            sys.stderr.write('F  ')
            sys.stderr.write(str(test))
...
class BeautifulReport(ReportTestResult, PATH):
            ...
            def __wrap(*args, **kwargs):
                ...
                if len(pargs) > 1:
                    for parg in pargs:
                        ...
                        print(HTML_IMG_TEMPLATE.format(data, data))
                    return result
                if not os.path.exists(os.path.join(img_path, '{}.png'.format(pargs[0]))):
                    return result
                data = BeautifulReport.img2base(img_path, pargs[0] + '.png')
                print(HTML_IMG_TEMPLATE.format(data, data))
                return result
    ...
    def depend_on(*pargs):
        def wrap(function):
            ...
            def _wrap(self):
                depend = pargs[0]
                if depend == function.__name__:
                    raise ValueError("{} can't depend on itself.".format(depend))
                failures = str([fail[0] for fail in self._outcome.result.failures])
                errors = str([fail[0] for fail in self._outcome.result.errors])
                skipped = str([fail[0] for fail in self._outcome.result.skipped])
                flag = (depend in failures) or (depend in errors) or (depend in skipped)
                test = unittest.skipIf(flag, '{} failed or error or skipped'.format(depend))(function)
                if flag:
                    print('{} failed or error or skipped'.format(depend))
                return test(self)
            return _wrap
        return wrap